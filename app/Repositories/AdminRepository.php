...
final class AdminRepository
{
    ...
    public function saveAluno(array $payload): int
    {
        ...
        try {
            if (!empty($payload['id'])) {
                $sql = 'UPDATE alunos SET nome = :nome, cpf = :cpf, data_nascimento = :data_nascimento, telefone = :telefone, email = :email, ativo = :ativo WHERE id = :id';
                $stmt = $pdo->prepare($sql);
                $stmt->bindValue(':id', $payload['id'], PDO::PARAM_INT);
            } else {
                $sql = 'INSERT INTO alunos (nome, cpf, data_nascimento, telefone, email, ativo) VALUES (:nome, :cpf, :data_nascimento, :telefone, :email, :ativo)';
                $stmt = $pdo->prepare($sql);
            }
