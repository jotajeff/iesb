  ...
  <nav class="navbar navbar-expand-lg" style="background: #1f2937 !important; border-bottom: 0; position: sticky; top:0; z-index:1100;">
    <div class="container">
      <a class="navbar-brand text-white" href="/admin">
        <span class="d-inline-flex align-items-center justify-content-center rounded-circle" style="width: 44px; height: 44px; background: rgba(255,255,255,0.15); backdrop-filter: blur(2px);">
          <img src="/assets/img/logo-top-admin.png" alt="Logo" class="img-fluid" style="max-height: 32px;">
        </span>
      </a>
      ...
      <div class="collapse navbar-collapse" id="adminNavbar">
        <ul class="navbar-nav me-auto">
          ...
          <?php else: ?>
            ...
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle <?= ((strpos((string) ($currentRoute ?? ''), '/admin/tarefas') === 0) || (strpos((string) ($currentRoute ?? ''), '/admin/matriculas/notificacao') === 0) || (strpos((string) ($currentRoute ?? ''), '/admin/chamadas') === 0) || (strpos((string) ($currentRoute ?? ''), '/admin/material') === 0) || (strpos((string) ($currentRoute ?? ''), '/admin/protocolos') === 0) || (strpos((string) ($currentRoute ?? ''), '/admin/documentos') === 0)) ? 'active' : '' ?>" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-folder2-open me-1"></i>Secretaria</a>
              <ul class="dropdown-menu">
                <li><span class="dropdown-item-text fw-bold"><i class="bi bi-kanban me-1"></i>Tarefas</span></li>
                <li><a class="dropdown-item" href="/admin/tarefas" style="padding-left:2.2rem;"><i class="bi bi-kanban me-1"></i>Kanban</a></li>
                <li><a class="dropdown-item" href="/admin/tarefas/lista" style="padding-left:2.2rem;"><i class="bi bi-list-ul me-1"></i>Lista</a></li>
                ...
                <li><a class="dropdown-item" href="/admin/protocolos"><i class="bi bi-chat-left-text me-1"></i>Protocolos</a></li>
                <li><a class="dropdown-item" href="/admin/documentos"><i class="bi bi-folder2-open me-1"></i>Documentos</a></li>
                ...
                <li><a class="dropdown-item" href="/admin/material"><i class="bi bi-folder2-open me-1"></i>Material</a></li>
            ...
            <?php
            $podeAsaas = $userRole === 'admin';
            if (!$podeAsaas) {
              $userIdMenu = (int) ($authUser['id'] ?? 0);
              if ($userIdMenu > 0) {
                $pdoMenu = \App\Core\Database::connection();
                if ($pdoMenu instanceof \PDO) {
                  try {
                    $stmtMenu = $pdoMenu->prepare('SELECT id FROM usuarios WHERE id = :id AND tipo = :tipo AND ativo = 1 LIMIT 1');
                    $stmtMenu->bindValue(':id', $userIdMenu, \PDO::PARAM_INT);
                    $stmtMenu->bindValue(':tipo', 'admin', \PDO::PARAM_STR);
                    $stmtMenu->execute();
                    $podeAsaas = (bool) $stmtMenu->fetch();
                  } catch (\Throwable $e) {
                    error_log('[MENU ASAAS] Erro: ' . $e->getMessage());
                  }
                }
              }
            }
            ...
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle <?= ((strpos($currentRoute ?? '', '/admin/sessao') === 0) || (strpos($currentRoute ?? '', '/admin/config/carousel') === 0) || (strpos($currentRoute ?? '', '/admin/config/noticias') === 0) || (strpos($currentRoute ?? '', '/admin/config/banner-aluno') === 0)) ? 'active' : '' ?>" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-file-earmark-text me-1"></i>Conteúdo</a>
              <ul class="dropdown-menu">
                ...
                <li><a class="dropdown-item" href="/admin/config/banner-aluno"><i class="bi bi-easel me-1"></i>Banner-Aluno</a></li>
                <li><a class="dropdown-item" href="/admin/config/noticias"><i class="bi bi-newspaper me-1"></i>Notícias</a></li>
              </ul>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle <?= ((strpos($currentRoute ?? '', '/admin/modalidade') === 0) || (strpos($currentRoute ?? '', '/admin/segmento') === 0) || (strpos($currentRoute ?? '', '/admin/setor') === 0) || (strpos($currentRoute ?? '', '/admin/nivel') === 0) || (strpos($currentRoute ?? '', '/admin/config/categoria') === 0) || (strpos($currentRoute ?? '', '/admin/config/cliente') === 0) || (strpos($currentRoute ?? '', '/admin/config/email') === 0) || (strpos($currentRoute ?? '', '/admin/funcoes-docente') === 0) || (strpos($currentRoute ?? '', '/admin/storage') === 0) || (($currentRoute ?? '') === '/admin/dbase') || (strpos($currentRoute ?? '', '/admin/permissoes') === 0)) ? 'active' : '' ?>" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-gear me-1"></i>Setup</a>
              <ul class="dropdown-menu">
                ...
                <li><a class="dropdown-item" href="/admin/config/email"><i class="bi bi-envelope me-1"></i>Email</a></li>
                ...
                <?php endif; ?>
                <li><a class="dropdown-item" href="/admin/permissoes"><i class="bi bi-shield-check me-1"></i>Permissões</a></li>
              </ul>
            </li>
          <?php endif; ?>
        </ul>
