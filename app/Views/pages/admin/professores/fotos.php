...
<section class="container py-4">
  <div class="bg-white border rounded-3 p-4 shadow-sm">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
      <h4 class="mb-0"><i class="bi bi-camera me-2"></i>Fotos — <?= htmlspecialchars((string) ($professor['nome'] ?? ''), ENT_QUOTES, 'UTF-8') ?></h4>
    ...
    <?php if ($flashUpload && (strpos($flashUpload, 'sucesso') !== false || $flashUpload === 'Foto salva com sucesso.')): ?>
      <div class="alert alert-success d-flex justify-content-between align-items-center">
        ...
        <span>
          <a class="btn btn-sm btn-outline-primary me-2" href="/admin/professores/fotos?id=<?= $idFk ?>"><i class="bi bi-plus-lg me-1"></i>Adicionar outra</a>
          <a class="btn btn-sm btn-outline-secondary" href="/admin/professores"><i class="bi bi-list-ul me-1"></i>Voltar à listagem</a>
        </span>
      </div>
    <?php endif; ?>
    ...
    <form method="post" action="/admin/professores/upload-foto" enctype="multipart/form-data">
...
function deletarFoto(id, idFk) {
  ...
  fetch('/admin/professores/deletar-foto', { method: 'POST', body: formData })
    .then(r => r.json())
    .then(data => {
      if (data.sucesso) {
        location.reload();
